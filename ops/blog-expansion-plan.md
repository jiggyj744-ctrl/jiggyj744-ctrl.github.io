...
정기 발행은 직접 OpenAI API 과금 호출이 아니라 VM109의 Antigravity/Gemini CLI 프록시를 OpenAI-compatible API처럼 호출하는 구조로 운영한다.
...
- 기존 저장소 상태: 정기 발행 엔진은 프록시 호출, JSON 파싱 보정, QA 실패 시 1회 재생성, 템플릿 fallback, 발행 원본 JSON 보존, sitemap/feed/blog hub/category 갱신, 저수량 백로그 자동 보충을 지원한다.

## 운영 구조
1. `content/blog-backlog.json`에 발행 대기 키워드를 쌓는다.
2. `.github/workflows/continuous-indexing.yml`은 하루 여러 후보 시간 중 KST 날짜 해시로 선택된 1개 슬롯만 통과시킨 뒤 공통 발행 스크립트를 호출한다.
3. 실행 위치는 GitHub hosted runner가 아니라 `vm109` 라벨이 붙은 VM109 self-hosted runner를 기준으로 한다.
4. VM109 runner는 `http://127.0.0.1:8302/v1`로 Gemini CLI 프록시를 호출한다.
5. `ops/vm109-share-blog-publisher.sh`가 lock 파일과 KST 날짜 기준 daily guard를 적용해 GitHub Actions와 systemd timer의 동시 발행을 막는다.
6. `scripts/seo-content-engine.mjs`가 우선순위가 높은 queued/improve 항목을 선택해 글을 생성한다.
7. queued/improve 블로그 항목이 20개 미만이면 지역·상황 조합 후보를 40개 수준까지 자동 보충한다.
8. 생성 결과가 JSON 형식을 어기거나 금지 문구 검수에 걸리면 QA 피드백을 반영해 1회 재생성하고, 그래도 실패할 때 템플릿 fallback으로 발행해 루프가 멈추지 않게 한다.
9. 발행 후 `/blog/`, `/blog/category/*/`, `/feed.xml`, `sitemap.xml`, 메인 최신 블로그 링크, `content/blog-posts/*.json` 원본을 갱신하고 커밋·푸시한다.

## 필수 설정
- GitHub Actions runner: VM109에 self-hosted runner 등록 후 `vm109` 라벨 부여.
...
- fallback 운영: 프록시 응답이 깨지거나 재생성 후에도 QA 실패가 남으면 자동 템플릿 발행으로 전환하되, `ops/index-state.json`에 실행 기록을 남긴다.
...
- 매도 의사를 흔들 수 있는 세부 실행 정보, 내부 판단 기준, 상대방 대응 방향은 공개하지 않는다.
- 공개 글은 접수 전 확인자료와 보류 가능성 안내에 한정하고, 후속 절차를 단정하는 표현은 블로그 본문에서 제외한다.
...
- `/blog/` 허브, 활성 카테고리 페이지, 메인 페이지에 최신 글 링크를 반영해 내부 링크를 만든다.
- 생성 원본은 `content/blog-posts/*.json`에 저장해 디자인 재빌드 때 본문이 fallback으로 덮이지 않게 한다.
...
3. 첫 7일은 랜덤 후보 슬롯과 최대 30분 지터 기준 1일 1건 발행으로 품질과 색인 반응을 본다.
...
- `node tools/verify_site.mjs`가 통과해야 한다. 이 검증은 발행된 모든 블로그 글, 원본 JSON, 카테고리 페이지, sitemap/feed 반영을 자동 확인한다.
- 배포 후 `node tools/verify_live.mjs`가 sitemap과 blog-backlog 기준 공개 URL 전체를 확인해야 한다.
...
## 재시작 복구 계획
서버 재부팅 후에도 발행 루프가 자동 재개되도록 `ops/autopublish-resilience-plan.md`에 systemd service/timer, lock, health check, 공개 URL 확인 구조를 별도로 정리했다.
