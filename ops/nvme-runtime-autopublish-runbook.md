...
```bash
LLM_PROXY_BASE_URL=http://192.168.0.46:8302/v1
...
현재 CT104 런타임에서 CT109 `automation-runtime` proxy는 `192.168.0.46:8302`로 접근한다. proxy를 같은 CT 안에서 돌리는 구성으로 바꾸면 `127.0.0.1:8302`로 되돌린다.
...
- proxy 인증은 통과해도 GitHub push credential이 없으면 발행물 생성 전 종료한다.
