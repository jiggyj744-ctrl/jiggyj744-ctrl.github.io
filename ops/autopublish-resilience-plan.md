...
## 목표
VM109 또는 호스트가 재시작되어도 지분매입 블로그 발행이 자동으로 재개되게 한다. 발행 루프는 Gemini CLI 프록시, 백로그 자동 보충, 원본 JSON 보존, 정적 사이트 검증, 커밋·푸시, 공개 URL 확인, 라이브 전체 검증까지 통과한 경우만 성공으로 본다.
...
2. VM109에 GitHub Actions self-hosted runner를 systemd 서비스로 등록하고 `vm109` 라벨을 부여한다.
...
9. 발행 후 `node tools/verify_site.mjs`, git diff, sitemap/feed/blog hub/category 반영, 공개 URL 200 확인, `node tools/verify_live.mjs` 재시도를 수행한다.
...
- 필요 조건: VM109 runner 등록 토큰, runner label `vm109`, repo secret `LLM_PROXY_API_KEY`, repo variable `LLM_PROXY_BASE_URL`, `LLM_PROXY_MODEL`
...
JAUCTION_PUBLISH_LOCK=/tmp/jauction-share-blog-publisher.lock
JAUCTION_PUBLISH_LOG_DIR=/tmp/jauction-share-blog-publisher
```
...
- 신규 글 URL이 200
- `/blog/`, `/blog/category/*/`, `/feed.xml`, `/sitemap.xml`에 신규 URL 반영
- `content/blog-posts/*.json`에 신규 글 원본 저장
- GitHub 원격 main에 커밋 반영
- `ops/index-state.json`에 실행 기록 저장
- `node tools/verify_live.mjs`가 sitemap과 blog-backlog 기준 공개 URL 전체 200 확인
...
- runner 대기 상태: GitHub Actions runner에 `vm109` 라벨이 없으면 workflow가 대기하므로 runner labels를 확인
