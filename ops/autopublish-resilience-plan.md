...
3. GitHub Actions schedule이 매일 06:12 KST에 1차 발행을 시도한다.
4. 별도 systemd timer가 같은 스크립트를 백업 경로로 실행한다.
5. lock 파일로 GitHub Actions와 timer가 동시에 발행하지 못하게 막는다.
6. 발행 전 Gemini proxy health와 실제 completion을 확인한다.
7. 발행 후 `node tools/verify_site.mjs`, git diff, sitemap/feed/blog hub 반영, 공개 URL 200 확인을 수행한다.
8. 실패하면 다음 timer 또는 다음 schedule에서 같은 대기열을 다시 처리한다.
